...
### Paint Transformer
**Paint transformer** is a novel Computer Vision Algorithm which converts a static picture into an artistic oil painting and show all the stroke sequences of the painting. The [Demo](#Dataset-Selection) section shows my reimplementing of this algorithm according to their [paper](https://arxiv.org/abs/2108.03798).

![VQGAN]({{ '/assets/images/team02/painttransformer.jpg | relative_url }})
{: style="width: 100%; max-width: 100%;"}
*Fig 5. Framework of Paint Transformer[2]*.

### Paint Transformer Framework
The structure of the whole training pipeline is provided in the Figure x. The Paint Transformer contains two parts: Stroke Predictor and Stroke Renderer. For each rendering iteration, the Stroke Predictor will give predictions from the input image I<sub>t</sub>, and pass the result to the stroke render. The stroke render will add strokes accordingly from the Canvas images I<sub>c</sub>. The result will be the image I<sub>r</sub>. Both stroke (direction and sequence) and pixel (color) will affect result. The loss function contains the two factors are presented below:

![equation](https://latex.codecogs.com/svg.image?\mathcal{L}&space;=&space;\mathcal{L}_{\text{stroke}}(S_r,S_f)&plus;\mathcal{L}_{\text{pixel}}(I_r,I_t)&space;\newline&space;)
...
### Stroke Render
Figure x+1 showed the mechanism of a Stroke Render which takes a predefined brush prototype and transforms it into expected stroke by applying different coordinates (x,y), height(h), width(w), rotate angle($\theta$), and color (r,g,b). The Render only contains linear transformation, so the render is not trainable.
...
```
### Dataset selection
In the paper, authors use a novel way to train the model with existing datasets. According to mechanism of this project, the loss function is the comparison between the generated strokes and the original image. With dependencies on labels, randomly generated images can be used for training. Since there is no limit of generating random images, the dataset can be really large for training more reliable models. The following snippet shows the generation of a random image. `foreground` and `alpha` inside this code snippet are generated strokes.
```python
...
#### Dataset Selection:
- Model privided in the original [repository](https://github.com/wzmsltw/PaintTransformer)
...
For each dataset, we trained a Paint Transformer Model for about 3 hours on a RTX 3080 graphics card. Below are generated from 120 epochs.
...
| :---        |    :----:   |          ---: |
| Model provided      | ![0](/assets/images/team02/model.jpg)       | ![0](/assets/images/team02/model.gif)   |
| No-dataset approach    | ![0](/assets/images/team02/original.jpg)       | ![0](/assets/images/team02/original.gif)   |
Miniplaces  |  | ![0](/assets/images/team02/model2.gif)
Landscape  |  | ![0](/assets/images/team02/model3.gif)



#### Summary:
Among all three results, the result from no dataset approach actually performed the best. Our implementation confirmed the statement in the [paper](https://arxiv.org/abs/2108.03798) that the no-dataset approach can achieve similar performance without extra training time.
## Demo
